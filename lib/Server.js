...
function onConnect(srv,socket) {
	...
	srv.socket.rl.on('line',function(str) {
		if(Number(process.version.split('.')[1]) < 10) {
...
function onJoin(srv,cname,joined,userhost) {
	...
	if(!srv.users[joined.toUpperCase()])	
		srv.users[joined.toUpperCase()] = new User(joined,joined,userhost);
	else 
		srv.users[joined.toUpperCase()].uh=userhost;
