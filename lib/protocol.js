...
function processQuery(srv,user,args) {
	...
	/* split separated commands */
	var cmds = getCommands(srv,args);

	/* emit command events */
	while(cmds.length > 0) {
		emitter.emit("Command",srv,user.nick,user,cmds.shift());
	}
}
/* handle privmsg */
function processPrivmsg(srv,user,args) {
	...
	/* emit command events */
	while(cmds.length > 0) {
		emitter.emit("Command",srv,target,user,cmds.shift());
	}
}
...
function matchPrefix(srv,args) {
	...
	if(srv.prefix == undefined)
		return args;
	var p = srv.prefix.toUpperCase();
	if(args[0][0] == p) {
		args[0] = args[0].substr(1);
		return args;
	}
	else if(args[0].toUpperCase() == p) {
		return args.slice(1);
	}
	return false;
}
...
/* get multiple commands, if possible */
function getCommands(srv,args) {
	var str = args.join(" ");
	var cmds = str.split(srv.separator)
